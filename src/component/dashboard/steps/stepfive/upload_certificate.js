...
const CertificateUploadForm = ({handleUploadCertificate}) => (
  ...
  <form className="flex justify-center items-center px-16 py-8 w-full font-semibold rounded-lg border-2 border-solid bg-sky-600 bg-opacity-10 border-neutral-300 max-w-[745px] max-md:px-5 max-md:max-w-full">
    <div className="flex text-center flex-col max-w-full ">
      ...
      <div className="mt-1.5 text-xs font-medium text-zinc-500">
        Click the button below to upload your certificate
      </div>
      ...
      <input type="file" id="uploadCertificate"  hidden onChange={e=>handleUploadCertificate(e.target.files[0])} />
      
    </div>
   </form>
);

const ActionButtons = ({handleUpdateProfile}) => (
...
const UploadCertificate = ({certificateFile,setCertificateFile,handleUpdateProfile}) => {
  ...
  return (
    ...
    <CertificateUploadForm handleUploadCertificate={handleUploadCertificate} />
    {show === true ? <ProgressBarF /> : <ProgressBar />}
      <ActionButtons handleUpdateProfile={handleUpdateProfile} />
  </section>
  )
}
